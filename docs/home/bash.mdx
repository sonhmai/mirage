...
`find -exec` runs each invocation in an isolated child shell. Mirage also
allows shell builtins and functions there; changes to their variables,
working directory, arguments, and shell options stay in the child.
...
`-exec` also refuses placement under `-o`, negation, or parentheses.
...
and `@1700000000`, with unzoned dates interpreted as UTC.
...
- **Shell variables:** `$?`, `$#`, `$@`, `$*`, `$0`..`$9`, `$$`, `$!`, `$RANDOM` (bash 5.2's generator, so `RANDOM=42` draws bash's own sequence; assigning seeds it, `unset RANDOM` strips it, as in bash) and `${PIPESTATUS[@]}` (the per-segment statuses of the last pipeline).
