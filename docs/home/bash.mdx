...
embedded newlines, across mount boundaries.
...
- **Shell variables:** `$?`, `$#`, `$@`, `$*`, `$0`..`$9`, `$$`, `$!`, `$RANDOM` (bash 5.2's generator, so `RANDOM=42` draws bash's own sequence; assigning seeds it immediately; invalid arithmetic seeds emit a diagnostic without changing the sequence or failing the assignment; `unset RANDOM` strips it, as in bash) and `${PIPESTATUS[@]}` (the per-segment statuses of the last pipeline).
