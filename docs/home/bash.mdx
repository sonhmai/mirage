...
- **Redirects:** `>`, `>>`, `>|`, `<`, `2>`, `2>&1`, `>&2`, `&>`, `&>>`, `>&-` (a closed stdout fails the write, as GNU echo reports), heredoc `<<`, herestring `<<<`. The shell models descriptors 0, 1 and 2 only: a redirect naming any other (`3>f`, `>&3`, `exec 3>&-`) is refused with `3: Bad file descriptor` and exit `1`, the line continuing as it would after any redirect error.
