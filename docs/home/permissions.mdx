...
is two questions: one `once` grant answers one spelling.
...
The ask example runs one guarded `rm` under both kinds of host: an
`on_ask` reviewer that allows it once inline, so the same line asks
again, and a ledger answered out of band by id with a session grant, so
nothing asks again:
