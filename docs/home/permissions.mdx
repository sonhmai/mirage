...
offer to a line judged at the same time. A grant given to a line that
launches a background job stays with the job until the job ends, since
its commands reach their gates after the line has returned. A line the
