...
being asked twice, and is spent by it.
...
The ask example runs one guarded line under both kinds of host, a
scripted `on_ask` answering inline and a ledger answered out of band by
id, and pins what allow-once, deny and allow-for-the-session each do to
the line that asked and to the next one:
