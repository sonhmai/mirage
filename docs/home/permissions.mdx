...
the line ends, for the same reason. A grant is bound to the place on
...
for the commands inside a background job it launches are the job's own
from the moment it is launched, and stay with it until it ends: its
commands reach their gates after the line has returned, and a question
that holds the rest of the line does not let go of them. This also
applies when `eval` launches the job: in
`eval 'sleep 1 && cat secret.txt &'`, the outer line's approval stays
reserved for the background `cat` after `eval` returns. A line the
shell evaluates from inside another (`$( )`, `eval`, `source`, `xargs`)
spends what the outer line was given for the words it runs, so
`echo $(cat secret.txt)` is one question, not two, and two such bodies on
one line are two. A line a whole-line runtime takes is judged the same
...
back earlier work, so retrying such a line can repeat it.
