...
Cancellation itself has one seam. Python cancels the command-tree task and joins it, so every await below it unwinds before `execute` raises. A JavaScript promise cannot be cancelled, so TypeScript fires the signal, joins the tree for a short grace (250 ms) while responsive leaves close their producers and settle, then releases the caller. `execute` always rejects with an `AbortError` whose `cause` is the signal's reason. A leaf that does not observe the signal within the grace (a custom handler, a backend call with no signal parameter) keeps running in the background until it settles on its own; its output is dropped and it cannot change `$?` (the status door refuses a statement that settles after the release), but any write it completes still lands on the backend. Loading workspace state, parsing, routing, the history record and the session flush are raced against the signal as well, so a stalled state store cannot hold the caller either.
