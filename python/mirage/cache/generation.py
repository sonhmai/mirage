...
class Generations:
    ...
    store-wide epoch answers ``clear`` and a prefix eviction, whose
    victims cannot be enumerated (a fill in flight has no entry yet).
    The per-key generation answers a removal of one key, so a large fill
    for one key is not thrown away because an unrelated key was written.
    Per-key generations exist only while a writer for that key is in
    flight, which bounds the map by concurrent writers, not by every key
    ever removed.
    """
