...
async function* wcLinesStream(source: AsyncIterable<Uint8Array>): AsyncIterable<Uint8Array> {
  ...
  yield formatRecords([String(count)])
}
...
export async function wcGeneric(
  ...
  if (LFlag) {
    const maxLen = text.split(/\r?\n/).reduce((m, l) => Math.max(m, l.length), 0)
    return [formatRecords([String(maxLen)]), new IOResult()]
  }
  if (wFlag) return [formatRecords([String(wcVal)]), new IOResult()]
  if (mFlag) return [formatRecords([String(cc)]), new IOResult()]
  if (cFlag) return [formatRecords([String(bc)]), new IOResult()]
  const line = formatWcLines([{ values: [lc, wcVal, bc], label: null }])[0] ?? ''
  return [formatRecords([line]), new IOResult()]
}
