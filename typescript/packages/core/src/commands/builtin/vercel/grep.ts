...
import { read as vercelRead } from '../../../core/vercel/read.ts'
...
async function grepCommand(
  ...
  if (paths.length > 0) {
    ...
    const rd = (p: string): Promise<string[]> =>
      vercelReaddir(accessor, toScope(p), opts.index ?? undefined)
    const st = (p: string): Promise<FileStat> =>
      vercelStat(accessor, toScope(p), opts.index ?? undefined)
    const rb = (p: string): Promise<Uint8Array> =>
      vercelRead(accessor, toScope(p), opts.index ?? undefined)
    ...
    const io = new IOResult({
      reads: { [target.stripPrefix]: data },
      cache: [target.stripPrefix],
    })
...
export const VERCEL_GREP = command({
  name: 'grep',
