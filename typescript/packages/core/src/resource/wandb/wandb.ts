...
import { WANDB_PROMPT } from '../../resource/wandb/prompt.ts'
...
export interface WandbResourceState {
  type: string
  config: WandbConfigRedacted
}

export class WandbResource extends BaseResource implements Resource {
  readonly kind: string = ResourceName.WANDB
  ...
  override getState(): Promise<WandbResourceState> {
    return Promise.resolve({
      type: this.kind,
  ...
  override loadState(_state: WandbResourceState): Promise<void> {
    return Promise.resolve()
  }
}
