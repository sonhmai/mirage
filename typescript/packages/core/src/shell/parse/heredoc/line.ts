 ...
 * run to their balancing paren; a `#` opening a word, which is one after
 * a blank or a metacharacter (`cat <<EOF;# don't`), starts a comment that
 * ends at the newline. A trailing `|` or `&&` does not extend the line:
 * bash gathers the body at the first newline and reads the rest of the
 * pipeline after the terminator. Returns null when the line never ends.
 */
